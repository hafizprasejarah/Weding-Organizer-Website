<nav class="flex w-full py-4 px-20 justify-between  bg-gradient-to-l from-[#ff6186] to-[#C63D5C] text-[#FFF5F7]">
    <a href="<?= base_url('/') ?>"><h1 class="font-semibold text-[24px]">Lovely Moments</h1></a>
    <ul class="flex gap-10 items-center font-medium">
        <li><a class="text-[#FFF5F7]" href="<?= base_url('/') ?>">Home</a></li>
        <li><a class="text-[#FFF5F7]" href="<?= base_url('/services') ?>">Services</a></li>
        <li><a class="text-[#FFF5F7]" href="<?= base_url('/gallery') ?>">Gallery</a></li>
        <li><a class="text-[#FFF5F7]" href="<?= base_url('/gallery') ?>">Packages</a></li>
        <li><a class="text-[#FFF5F7]" href="<?= base_url('/about') ?>">Contact</a></li>
        <li><a class="text-[#FFF5F7]" href="<?= base_url('/contact') ?>"><Button>BOOK NOW</Button></a></li>
    </ul>
</nav>